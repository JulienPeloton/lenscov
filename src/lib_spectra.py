...
class camb_clfile(object):
	...
	def __init__(self, tfname, lmax=None):
		...
		if tfname.split('.')[0].split('_')[-1] == 'lenspotentialCls':
			...
			self.clpp = np.concatenate( [ np.zeros(lmin), tarray[0:(lmax-lmin+1),5]*2.*np.pi/(ell*(ell+1))**2/tcmb**2 ] )
			...
			self.clpp_long = np.concatenate( [ np.zeros(lmin), tarray[0:(lmax_spec-lmin+1),5]*2.*np.pi/(ell_spec*(ell_spec+1))**2/tcmb**2 ] )
