*DECK WIG6J
      SUBROUTINE WIG6J (L2, L3, L4, L5, L6, L1MIN, L1MAX, SIXCOF, NDIM,
     +   IER)
C***BEGIN PROLOGUE  WIG6J
...
C***LIBRARY   SLATEC
C***CATEGORY  C19
C***TYPE      DOUBLE PRECISION (RC6J-S, WIG6J-D)
...
C        INTEGER NDIM, IER
C
C        CALL WIG6J(L2, L3, L4, L5, L6, L1MIN, L1MAX, SIXCOF, NDIM, IER)
C
C *Arguments:
C
...
C
C***FIRST EXECUTABLE STATEMENT  WIG6J
      IER=0
...
C  Check error condition 4.
      IF(MOD(L1MAX-L1MIN+EPS,ONE).GE.EPS+EPS)THEN
         IER=4
         CALL XERMSG('SLATEC','WIG6J','L1MAX-L1MIN not integer.',IER,1)
         RETURN
      ENDIF
...
C  Check error condition 5.
      IER=5
      CALL XERMSG('SLATEC','WIG6J','L1MIN greater than L1MAX.',IER,1)
      RETURN
C
C
...
C  Check error condition 6.
   21 IER = 6
      CALL XERMSG('SLATEC','WIG6J','Dimension of result array for 6j '//
